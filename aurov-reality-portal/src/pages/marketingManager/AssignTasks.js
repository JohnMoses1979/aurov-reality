...
const TITLE = 'Marketing Assign Tasks';
const workflowApi = departmentManagerWorkflowApi.marketing;

export default function MarketingManagerAssignTasks() {
  ...
  const loadData = async () => {
    try {
      ...
      setMessage(error.message || 'Unable to load marketing task data.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const submitTask = async (event) => {
    ...
    try {
      ...
      const assigned = await workflowApi.assign({
  ...
  const downloadPdf = async (task) => {
    try {
      setMessage('');
      await workflowApi.downloadPdf(task);
    } catch (error) {
