...
import { crmExecutiveTaskUpdatesApi } from '../../services/api.js';

const TITLE = 'My Task Updates';

export default function CrmExecutiveMyTaskUpdates() {
  ...
  useEffect(() => {
    const loadUpdates = async () => {
      try {
        const data = await crmExecutiveTaskUpdatesApi.getMine();
  ...
  const downloadPdf = async (task, kind) => {
    try {
      await crmExecutiveTaskUpdatesApi.downloadPdf(task, kind);
    } catch (error) {
      console.error(error);
    }
  };
