...
import { crmExecutiveTaskApi } from '../../services/api.js';

const TITLE = 'Assigned Tasks';

export default function CrmExecutiveAssignedTasks() {
  ...
  const loadTasks = async () => {
    try {
      ...
      const data = await crmExecutiveTaskApi.getMine();
  ...
  const updateStatus = async (task, status) => {
    try {
      const updatedTask = await crmExecutiveTaskApi.updateStatus(task.id, status);
      ...
      setMessage({
        ...
        text:
          status === 'Submitted'
            ? 'Work submitted to CRM manager successfully.'
  ...
  const upload = async (task, file, kind) => {
    ...
    try {
      const updatedTask = await crmExecutiveTaskApi.uploadPdf(task.id, file, kind);
      ...
      setMessage({
        ...
        text:
          kind === 'completion'
            ...
            : 'Work update submitted to CRM manager successfully.',
      });
    } catch (error) {
      setMessage({
        type: 'error',
  ...
  const downloadTaskPdf = async (task, kind) => {
    try {
      await crmExecutiveTaskApi.downloadPdf(task, kind);
    } catch (error) {
      setMessage({
        type: 'error',
  ...
  return (
    <div>
      ...
      <SectionCard
        ...
        subtitle="Download assigned PDF, submit update PDF to CRM manager, upload completion PDF, and update task status."
        action={
