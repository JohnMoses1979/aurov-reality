...
//     target: executiveUser ? 'Sales Manager' : 'Managing Director',
//     subject: '',
//     description: '',
//     priority: 'Medium',
//     attachments: [],
...
function openLocalAttachment(doc) {
  if (!doc?.file) return;
...
function downloadLocalAttachment(doc) {
  if (!doc?.file) return;
  ...
  link.href = url;
  link.download = doc.name || 'attachment';
...
export default function Complaints() {
  ...
  const roleComplaints = useMemo(
    () => complaints.map(normalizeComplaintForView),
    [complaints]
  );
  ...
  useEffect(() => {
    ...
    const latestComplaint = complaints.find(
      (item) => item.id === selectedComplaint.id
    );

    if (latestComplaint) {
      setSelectedComplaint(normalizeComplaintForView(latestComplaint));
    }
  ...
  const targetOptions = useMemo(() => {
    if (executiveUser) {
      return [
        {
          value: managerRole || 'Sales Manager',
          label: `Send to ${managerRole || 'Sales Manager'}`,
        },
        {
          value: 'Managing Director',
          label: 'Send to Managing Director',
        },
        {
  ...
  const summary = useMemo(() => {
    ...
    const escalated = all.filter(
      (item) =>
        ['Leadership', 'Managing Director', 'Operational Head'].includes(item.target) ||
        (item.audience || []).some((audience) =>
          ['Managing Director', 'Operational Head', 'Operations Head'].includes(
            audience
          )
        )
    ).length;
  ...
  const uploadAttachments = async (event) => {
    ...
    setForm((old) => ({
      ...old,
      attachments: [...old.attachments, ...docs],
    }));
  };
  ...
  const raise = async (event) => {
    ...
    try {
      const created = await complaintsApi.create(form, user);

      setComplaints((old) => [created, ...old]);
  ...
  const updateStatus = async (status = reviewStatus, note = reviewNote) => {
    ...
    try {
      ...
      const normalized = normalizeComplaintForView(updated);
  ...
  const updateComplaintFromTable = async (complaint, status, note) => {
    try {
      const updated = await complaintsApi.updateStatus(
        complaint.id,
        status,
  ...
  const previewAttachment = async (complaint, doc) => {
    try {
      if (doc.file) {
        openLocalAttachment(doc);
        return;
      }

      await complaintsApi.previewAttachment(complaint.id, doc.id);
    } catch (error) {
      setMessage(error.message || 'Unable to preview attachment.');
    }
  };

  const downloadAttachment = async (complaint, doc) => {
    try {
      if (doc.file) {
        downloadLocalAttachment(doc);
        return;
      }

      await complaintsApi.downloadAttachment(complaint.id, doc.id, doc.name);
    } catch (error) {
        ...
        subtitle={
          leadershipUser
            ...
            : canRaiseComplaint
              ...
              : 'Complaint records routed through backend.'
        }
      />
      ...
      >
        {canRaiseComplaint && (
          <SectionCard
            ...
            subtitle={
              executiveUser
                ? 'Send complaints to Sales Manager, Managing Director, or Operational Head.'
                : 'Managers can send complaints directly to Managing Director or Operational Head.'
            }
          >
            <form onSubmit={raise} className="space-y-4">
              <label className="block">
              ...
              <div className="rounded-2xl bg-slate-50 p-4 text-sm font-bold leading-6 text-slate-600">
                ...
                <p>
                  <b>Employee Name:</b> {user?.name || user?.fullName || role}
                </p>
                <p>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {form.attachments.map((doc) => (
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() => removeAttachment(doc.id || doc.name)}
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {selectedComplaint.attachments.map((doc) => (
                  ...
                  >
                    ...
                    <button
                      onClick={() =>
                        previewAttachment(selectedComplaint, doc)
                      }
                    ...
                    <button
                      onClick={() =>
                        downloadAttachment(selectedComplaint, doc)
                      }
