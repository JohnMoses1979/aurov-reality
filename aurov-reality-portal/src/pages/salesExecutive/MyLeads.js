...
// export default function SalesExecutiveMyLeads() {
//   const { user } = useAuth();
...
import PrimaryButton from '../../components/ui/PrimaryButton.js';
import StatusBadge from '../../components/ui/StatusBadge.js';
import { salesExecutiveLeadsApi } from '../../services/api.js';
...
export default function SalesExecutiveMyLeads() {
  ...
  const loadTasks = async () => {
    try {
      ...
      const data = await salesExecutiveLeadsApi.getMine();

      setTasks(Array.isArray(data) ? data : []);
    } catch (error) {
      setMessage(error.message || 'Unable to load assigned tasks.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateStatus = async (task, status) => {
    try {
      const updatedTask = await salesExecutiveLeadsApi.updateStatus(
        task.id,
        status
      );
  ...
  const upload = async (task, file, kind) => {
    ...
    try {
      const updatedTask = await salesExecutiveLeadsApi.uploadPdf(
        task.id,
        file,
        kind
      );
  ...
  const downloadPdf = async (task, kind) => {
    try {
      await salesExecutiveLeadsApi.downloadPdf(task, kind);
    } catch (error) {
  ...
  return (
    <div>
      <PageHeader
        title={TITLE}
        subtitle="All PDF work assigned by your manager. Update status and upload PDFs from this page."
      />
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-xs font-bold text-slate-400">
                    Assigned by {task.assignedByName || task.assignedBy || 'Manager'} ·
                    Due {task.due || '-'}
                  </p>
                </div>
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-[180px_1fr]">
                ...
                <div className="flex flex-wrap gap-2">
                  <PrimaryButton
                    variant="outline"
                    className="py-2"
                    onClick={() => downloadPdf(task, 'assignment')}
                  >
                  ...
                  >
                    <FiUploadCloud /> Upload Update PDF
                  </PrimaryButton>
                  ...
                  <input
                    ...
                    className="hidden"
                    onChange={(e) =>
                      upload(task, e.target.files?.[0], 'completion')
                    }
                  />
                </div>
              </div>
