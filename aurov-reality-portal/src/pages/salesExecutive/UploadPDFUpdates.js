...
// const TITLE = 'Upload PDF Updates';
...
// export default function SalesExecutiveUploadPDFUpdates() {
//   const { user } = useAuth();
...
import PrimaryButton from '../../components/ui/PrimaryButton.js';
import StatusBadge from '../../components/ui/StatusBadge.js';
import { salesExecutivePdfUpdatesApi } from '../../services/api.js';
...
export default function SalesExecutiveUploadPDFUpdates() {
  ...
  useEffect(() => {
    const loadUpdates = async () => {
      try {
        const data = await salesExecutivePdfUpdatesApi.getMine();
  ...
  const downloadPdf = async (task, kind) => {
    try {
      await salesExecutivePdfUpdatesApi.downloadPdf(task, kind);
    } catch (error) {
      console.error(error);
    }
  };
