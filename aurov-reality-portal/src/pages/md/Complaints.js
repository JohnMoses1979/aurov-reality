...
const departments = ['Sales', 'Marketing', 'CRM', 'Accounts', 'HR', 'Customers'];
const statuses = ['Pending', 'In Progress', 'Resolved', 'Closed'];
...
function normalizeComplaintForView(complaint) {
  ...
  const sourceType = complaint.sourceType || '';
...
export default function Complaints() {
  ...
  const [roleComplaints, setRoleComplaints] = useState([]);
  ...
  const loadComplaints = async () => {
    try {
      ...
      const params = {
        department: activeDepartment === 'All' ? '' : activeDepartment,
      ...
      setRoleComplaints((res.data || []).map(normalizeComplaintForView));
    } catch (error) {
      console.error(error);
      setMessage(
  ...
  const summary = useMemo(() => {
    ...
    const escalated = all.filter(
      (item) =>
        ...
        item.department === 'Customers' ||
        (item.audience || []).some((audience) =>
          ['Managing Director', 'Operational Head'].includes(audience)
        )
    ).length;
  ...
  const uploadAttachments = async (event) => {
    ...
    setAttachmentFiles((old) => [...old, ...files]);

    setForm((old) => ({
      ...old,
      attachments: [...old.attachments, ...previews],
    }));
  };
  ...
  const removeAttachment = (name) => {
    setForm((old) => ({
      ...old,
      attachments: old.attachments.filter((item) => item.name !== name),
    }));

    setAttachmentFiles((old) => old.filter((file) => file.name !== name));
  };
  ...
  const raise = async (event) => {
    ...
    try {
      ...
      data.append('employeeId', user?.employeeId || user?.id || '');
      data.append('employeeName', user?.name || role);
      data.append('employeeRole', role);

      attachmentFiles.forEach((file) => {
        data.append('attachments', file);
      });

      await complaintApi.createEmployeeComplaint(data);
  ...
  const updateStatus = async (status = reviewStatus, note = reviewNote) => {
    ...
    try {
      ...
      const updatedComplaint = normalizeComplaintForView(res.data);
  ...
  const updateComplaintDirectly = async (complaint, status, note) => {
    try {
      await complaintApi.updateStatus(complaint.id, status, note);
      setMessage(`Complaint ${status.toLowerCase()} successfully.`);
      await loadComplaints();
    } catch (error) {
      console.error(error);
      setMessage(
        error?.response?.data?.message ||
          error?.response?.data ||
          `Unable to update complaint as ${status}.`
      );
    }
  };
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={
          leadershipUser
            ? 'Department-wise complaint dashboard with customer complaints, filters, status tracking, and resolution workflow.'
            : canRaiseComplaint
              ? 'Raise complaints, track status, and review department complaint history.'
              : 'Complaint records from backend.'
        }
      />
      ...
      >
        ...
        <SectionCard
          ...
          subtitle="Use filters, department tabs, search, and status actions to manage complaints."
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
              <label className="flex items-center gap-2 rounded-2xl border border-slate-200 px-3 py-2 xl:col-span-2">
                ...
                <input
                  ...
                  placeholder="Search complaint, customer, subject, role..."
                />
              </label>
              ...
              <input
                ...
                className="rounded-2xl border border-slate-200 px-3 py-2 text-sm font-bold outline-none"
                placeholder="Employee / Customer Name"
              />
            ...
            <table className="min-w-[1180px] w-full text-left">
              <thead>
                <tr className="border-b border-slate-200 text-xs font-black uppercase tracking-wider text-slate-500">
                  <th className="py-4 pr-4">Complaint ID</th>
                  <th className="py-4 pr-4">Employee ID</th>
                  <th className="py-4 pr-4">Employee / Customer Name</th>
                  <th className="py-4 pr-4">Department</th>
                  <th className="py-4 pr-4">Role</th>
                  <th className="py-4 pr-4">Complaint Subject</th>
                  <th className="py-4 pr-4">Raised To</th>
            ...
            <div className="mb-5 flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm font-bold text-slate-500">
                  {selectedComplaint.complaintCode || selectedComplaint.id} ·{' '}
                ...
                {selectedComplaint.attachments.map((doc) => (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        complaintApi.downloadAttachment(
                          selectedComplaint.id,
                          doc.id,
                          doc.name
                        )
