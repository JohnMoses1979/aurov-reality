...
//       setMessage('Please enter work title, description, date, and department.');
//       return;
//     }
//     if (!file) {
...
//     setSubmitting(true);
//     try {
//       await app.submitManagerWork(form, file, user);
...
// export default function MdSubmittedWork() {
//   return <LeadershipSubmittedWorkPanel />;
// }
...
export function ManagerSubmitWorkPanel() {
    ...
    try {
      ...
      setMessage(
        error?.response?.data?.message ||
          'Unable to load submitted work history.'
      );
    } finally {
      setLoading(false);
    }
  ...
  const submitted = mySubmissions.filter(
    (item) => item.status === 'Submitted'
  ).length;

  const approved = mySubmissions.filter(
    (item) => item.status === 'Approved'
  ).length;

  const rejected = mySubmissions.filter(
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      await submittedWorkApi.submit(data);

      setMessage(
        'Work submitted successfully to Managing Director and Operational Head.'
      );
  ...
  const previewPdf = async (item) => {
    try {
      await submittedWorkApi.openPdf(item.id);
    } catch (error) {
      console.error(error);
  ...
  const downloadPdf = async (item) => {
    try {
      await submittedWorkApi.downloadPdf(
        item.id,
        item.pdf?.name || `${item.title || 'submitted-work'}.pdf`
      );
    } catch (error) {
      console.error(error);
  ...
  return (
    <div>
      <PageHeader
        title="Submit Work to MD/OH"
        subtitle="Submit completed department work with PDF attachment. Records are saved in backend and instantly visible to MD and Operational Head."
      />

      {message && (
        <div className="mb-5 rounded-2xl border border-blue-100 bg-blue-50 p-3 text-sm font-black text-[#0B3D91]">
          {message}
        </div>
      )}
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          label="Total Submitted Works"
          value={mySubmissions.length}
          icon={FiFileText}
          trend="My department submissions"
        />
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[430px_minmax(0,1fr)]">
        <SectionCard
          ...
          subtitle="Upload one PDF and submit work details to leadership."
        >
          <form onSubmit={submit} className="space-y-4">
            <label className="block">
              ...
              <input
                ...
                placeholder="Monthly sales closure report"
              />
            </label>
            ...
            <label className="block">
              ...
              <textarea
                ...
                rows="5"
                className="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm font-bold outline-none focus:border-[#1E5EFF]"
                placeholder="Explain completed work, outcomes, and next steps"
              />
            </label>
            ...
            <button
              ...
              className="flex w-full items-center justify-between gap-3 rounded-3xl border border-dashed border-[#0B7A8F]/40 bg-[#F0FDFA] p-4 text-left transition hover:border-[#0B7A8F]"
            >
              <div>
                <p className="font-black text-slate-900">PDF Attachment</p>
              ...
              <span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#0B3D91] text-white">
                <FiUploadCloud />
              </span>
            </button>
            ...
            <PrimaryButton
              ...
              disabled={submitting}
            >
              <FiSend /> {submitting ? 'Submitting...' : 'Submit Work'}
            </PrimaryButton>
          </form>
        </SectionCard>
        ...
        <SectionCard
          ...
          subtitle="Status updates from Managing Director and Operational Head."
        >
          <div className="table-wrap">
            <table className="min-w-[900px] w-full text-left">
              <thead>
...
export function LeadershipSubmittedWorkPanel() {
  ...
  const loadWorks = async () => {
    try {
      ...
      const apiMethod =
        submittedWorkApi.getLeadership || submittedWorkApi.getAll;

      if (!apiMethod) {
        throw new Error('Submitted work API is not available.');
      }
      ...
      setMessage(
        error?.message ||
          error?.response?.data?.message ||
          'Unable to load submitted work records.'
      );
      setWorks([]);
    } finally {
      setLoading(false);
  ...
  const loadCalendarActivities = async () => {
    try {
      const res = await submittedWorkApi.getCalendarActivities(selectedDate);

      setActivities({
        submitted: res.data?.submitted || [],
        bookings: res.data?.bookings || [],
        siteVisits: res.data?.siteVisits || [],
        demoRequests: res.data?.demoRequests || [],
  ...
  const managers = useMemo(
    () => [
      'All',
      ...Array.from(
        new Set(works.map((item) => item.managerName).filter(Boolean))
      ),
    ],
    [works]
  );
  ...
  const filteredWorks = useMemo(
    () =>
      works.filter((item) => {
        ...
        if (managerFilter !== 'All' && item.managerName !== managerFilter) {
          return false;
        }
        ...
        if (
          dateFilter &&
          !sameDate(
            item.submissionIso || item.submissionDate || item.createdIso,
            dateFilter
          )
        ) {
          return false;
  ...
  const activityTotals = [
    {
      label: 'Demo Requests',
      count: activities.demoRequests.length,
    },
    {
      label: 'Property Bookings',
      count: activities.bookings.filter(
        (item) => item.type === 'Reservation' || item.type === 'Purchase'
      ).length,
    },
    {
      label: 'Submitted Works',
      count: activities.submitted.length,
    },
    {
      label: 'Complaints',
      count: activities.complaints.length,
    },
  ];

  const review = async (item, status) => {
    try {
      setReviewingId(`${item.id}-${status}`);
      setMessage('');

      await submittedWorkApi.review(item.id, status, remarks[item.id] || '');
      ...
      setMessage(`Submitted work marked as ${status}.`);
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Review manager-submitted PDF work, approve or reject submissions, and monitor daily operational activities."
      />
      ...
      <SectionCard
        ...
        subtitle="Filter by department, status, manager, date, or search by submission ID / manager / employee ID."
        className="mt-6"
      >
        <div className="mb-5 grid gap-3 md:grid-cols-2 xl:grid-cols-6">
          <select
          ...
          <label className="flex items-center gap-2 rounded-2xl border border-slate-200 px-3 py-3 xl:col-span-2">
            ...
            <input
              ...
              placeholder="Search Submission ID, Manager, Employee ID..."
            />
          </label>
        </div>
            ...
            <tbody>
              {filteredWorks.map((item) => (
                ...
                >
                  ...
                  <td className="py-4 pr-4">
                    <p className="max-w-[220px] font-black text-slate-900">
                      {item.title}
                    </p>
                  </td>

                  <td className="py-4 pr-4">
                    <p className="max-w-[260px] leading-5 text-slate-500">
                      {item.description}
                    </p>
                  </td>
                  ...
                  <td className="py-4 pr-4">
                    <div className="space-y-2">
                      <input
                        ...
                        className="w-full min-w-[220px] rounded-2xl border border-slate-200 px-3 py-2 text-xs font-bold outline-none"
                      ...
                      <div className="flex flex-wrap gap-2">
                        <PrimaryButton
                          variant="outline"
                          className="py-2"
                          disabled={reviewingId === `${item.id}-Reviewed`}
                          onClick={() => review(item, 'Reviewed')}
                        >
                          {reviewingId === `${item.id}-Reviewed`
                            ? 'Saving...'
                            : 'Reviewed'}
                        </PrimaryButton>
                        ...
                        <PrimaryButton
                          variant="green"
                          className="py-2"
                          disabled={reviewingId === `${item.id}-Approved`}
                          onClick={() => review(item, 'Approved')}
                        >
                          {reviewingId === `${item.id}-Approved`
                            ? 'Approving...'
                            : 'Approve'}
                        </PrimaryButton>
                        ...
                        <PrimaryButton
                          variant="danger"
                          className="py-2"
                          disabled={reviewingId === `${item.id}-Rejected`}
                          onClick={() => review(item, 'Rejected')}
                        >
                          {reviewingId === `${item.id}-Rejected`
                            ? 'Rejecting...'
                            : 'Reject'}
                        </PrimaryButton>
                      </div>
        ...
        {filteredWorks.length === 0 && !loading && (
          <p className="rounded-2xl bg-slate-50 p-4 text-sm font-bold text-slate-500">
            No submitted work records found.
          </p>
        )}
      </SectionCard>
      ...
      <SectionCard
        title="Calendar View"
        subtitle="Select a date to view submitted work, bookings, site visits, demo requests, and complaints."
        className="mt-6"
      >
        <div className="mb-5 grid gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
          <div className="rounded-3xl bg-gradient-to-br from-[#0B3D91] to-[#12B76A] p-5 text-white">
            ...
            <input
              ...
              className="mt-3 w-full rounded-2xl border border-white/20 bg-white/15 px-3 py-3 text-sm font-black text-white outline-none [color-scheme:dark]"
            />
            ...
            <div className="mt-5 space-y-2">
              {activityTotals.map((item) => (
                <div
                  key={item.label}
                  className="flex justify-between rounded-2xl bg-white/10 px-3 py-2 text-sm font-black"
                >
              ...
              render={(item) => (
                ...
                >
                  ...
                  <span className="text-xs">
                    {item.managerName} · {item.department}
                  </span>
                </div>
              )}
            />
            ...
            <CalendarActivityList
              ...
              render={(item) => (
                ...
                >
                  <b>{item.name}</b>
                  <br />
                  <span className="text-xs">{item.venture}</span>
                </div>
              )}
            />
            ...
            <CalendarActivityList
              ...
              render={(item) => (
                ...
                >
                  ...
                  <br />
                  <span className="text-xs">
                    {item.employeeName || item.from} · {item.department}
                  </span>
                </div>
              )}
            />
