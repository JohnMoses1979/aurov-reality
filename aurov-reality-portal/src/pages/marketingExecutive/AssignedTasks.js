...
import { marketingExecutiveTaskApi } from '../../services/api.js';

const TITLE = 'Assigned Tasks';

export default function MarketingExecutiveAssignedTasks() {
  ...
  const loadTasks = async () => {
    try {
      ...
      const data = await marketingExecutiveTaskApi.getMine();
  ...
  const updateStatus = async (task, status) => {
    try {
      const updatedTask = await marketingExecutiveTaskApi.updateStatus(task.id, status);
      ...
      setMessage({
        ...
        text:
          status === 'Submitted'
            ? 'Work submitted to marketing manager successfully.'
  ...
  const upload = async (task, file, kind) => {
    ...
    try {
      const updatedTask = await marketingExecutiveTaskApi.uploadPdf(task.id, file, kind);
      ...
      setMessage({
        ...
        text:
          kind === 'completion'
            ...
            : 'Work update submitted to marketing manager successfully.',
      });
    } catch (error) {
      setMessage({
        type: 'error',
  ...
  const downloadTaskPdf = async (task, kind) => {
    try {
      await marketingExecutiveTaskApi.downloadPdf(task, kind);
    } catch (error) {
      setMessage({
        type: 'error',
  ...
  return (
    <div>
      ...
      <SectionCard
        ...
        subtitle="Download assigned PDF, submit update PDF to marketing manager, upload completion PDF, and update task status."
        action={
