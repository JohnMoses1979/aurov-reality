...
export default function EmployeeManagementPanel() {
  ...
  useEffect(() => {
    if (!editingId && creatableRoles.length && !creatableRoles.includes(form.role)) {
      ...
      setForm((old) => ({ ...old, role, department: getRoleDepartment(role) }));
    }
  }, [creatableRoles, editingId, form.role]);
        ...
        {canManage && roleOptions.length > 0 && (
          <SectionCard title={formTitle} subtitle={formSubtitle}>
            ...
            <form onSubmit={saveEmployee} className="space-y-5">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="block"><span className="mb-2 block text-sm font-extrabold text-slate-700">Password</span><input value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} className="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm font-bold outline-none focus:border-[#1E5EFF]" placeholder="FirstName@123" /><p className="mt-1 text-xs font-bold text-slate-500">Leave password empty to auto-generate FirstName@123, or enter your own password manually.</p></label>
