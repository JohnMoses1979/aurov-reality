...
import { hrExecutiveTaskApi } from '../../services/api.js';

const TITLE = 'Assigned Tasks';

export default function HrExecutiveAssignedTasks() {
  ...
  const loadTasks = async () => {
    try {
      ...
      const data = await hrExecutiveTaskApi.getMine();
  ...
  const updateStatus = async (task, status) => {
    try {
      const updatedTask = await hrExecutiveTaskApi.updateStatus(task.id, status);
      ...
      setMessage({
        ...
        text:
          status === 'Submitted'
            ? 'Work submitted to HR manager successfully.'
  ...
  const upload = async (task, file, kind) => {
    ...
    try {
      const updatedTask = await hrExecutiveTaskApi.uploadPdf(task.id, file, kind);
      ...
      setMessage({
        ...
        text:
          kind === 'completion'
            ...
            : 'Work update submitted to HR manager successfully.',
      });
    } catch (error) {
      setMessage({
        type: 'error',
  ...
  const downloadTaskPdf = async (task, kind) => {
    try {
      await hrExecutiveTaskApi.downloadPdf(task, kind);
    } catch (error) {
      setMessage({
        type: 'error',
  ...
  return (
    <div>
      ...
      <SectionCard
        ...
        subtitle="Download assigned PDF, submit update PDF to HR manager, upload completion PDF, and update task status."
        action={
