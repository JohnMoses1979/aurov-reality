...
import { hrExecutiveTaskUpdatesApi } from '../../services/api.js';

const TITLE = 'My Task Updates';

export default function HrExecutiveMyTaskUpdates() {
  ...
  useEffect(() => {
    const loadUpdates = async () => {
      try {
        const data = await hrExecutiveTaskUpdatesApi.getMine();
  ...
  const downloadPdf = async (task, kind) => {
    try {
      await hrExecutiveTaskUpdatesApi.downloadPdf(task, kind);
    } catch (error) {
      console.error(error);
    }
  };
