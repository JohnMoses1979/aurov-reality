...
import { accountsExecutiveTaskApi } from '../../services/api.js';

const TITLE = 'Assigned Tasks';

export default function AccountsExecutiveAssignedTasks() {
  ...
  const loadTasks = async () => {
    try {
      ...
      const data = await accountsExecutiveTaskApi.getMine();
  ...
  const updateStatus = async (task, status) => {
    try {
      const updatedTask = await accountsExecutiveTaskApi.updateStatus(task.id, status);
      ...
      setMessage({
        ...
        text:
          status === 'Submitted'
            ? 'Work submitted to accounts manager successfully.'
  ...
  const upload = async (task, file, kind) => {
    ...
    try {
      const updatedTask = await accountsExecutiveTaskApi.uploadPdf(task.id, file, kind);
      ...
      setMessage({
        ...
        text:
          kind === 'completion'
            ...
            : 'Work update submitted to accounts manager successfully.',
      });
    } catch (error) {
      setMessage({
        type: 'error',
  ...
  const downloadTaskPdf = async (task, kind) => {
    try {
      await accountsExecutiveTaskApi.downloadPdf(task, kind);
    } catch (error) {
      setMessage({
        type: 'error',
  ...
  return (
    <div>
      ...
      <SectionCard
        ...
        subtitle="Download assigned PDF, submit update PDF to accounts manager, upload completion PDF, and update task status."
        action={
