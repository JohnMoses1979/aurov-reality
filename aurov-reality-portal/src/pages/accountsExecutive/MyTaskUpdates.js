...
import { accountsExecutiveTaskUpdatesApi } from '../../services/api.js';

const TITLE = 'My Task Updates';

export default function AccountsExecutiveMyTaskUpdates() {
  ...
  useEffect(() => {
    const loadUpdates = async () => {
      try {
        const data = await accountsExecutiveTaskUpdatesApi.getMine();
  ...
  const downloadPdf = async (task, kind) => {
    try {
      await accountsExecutiveTaskUpdatesApi.downloadPdf(task, kind);
    } catch (error) {
      console.error(error);
    }
  };
