...
// import PrimaryButton from '../../components/ui/PrimaryButton.js';
// import StatusBadge from '../../components/ui/StatusBadge.js';
// import { getRoleDepartment } from '../../constants/roles.js';
// import { useAuth } from '../../context/AuthContext.js';
...
// const documentHelp =
//   'Supported: PDF, JPG, JPEG, PNG. Upload at least 2 documents. Recommended: Profile Photo, Aadhaar, PAN, Resume, Educational Certificate, Experience Certificate.';
...
//     user?.department || getRoleDepartment(user?.role || '')
//   );
//   const [roleOptions, setRoleOptions] = useState([]);

//   const [form, setForm] = useState(() => blankEmployee('Sales Executive'));
//   const [editingId, setEditingId] = useState(null);
...
//       setCurrentRole(data.currentRole || user?.role || '');
//       setCurrentDepartment(data.currentDepartment || '');
//       setRoleOptions(data.roleOptions || []);
//       setEmployeesRaw(data.employees || []);

//       if (!editingId) {
//         const firstRole = data.roleOptions?.[0] || 'Sales Executive';
//         setForm(blankEmployee(firstRole));
//       }
//     } catch (error) {
//       setMessage({
//         type: 'error',
//         text: error.message || 'Unable to load employees.',
//       });
//     } finally {
//       setLoading(false);
//     }
...
//   const reset = () => {
//     const role = roleOptions[0] || 'Sales Executive';
//     setForm(blankEmployee(role));
//     setEditingId(null);
//   };
...
//       department: getRoleDepartment(role),
//       designation:
//         old.designation === old.role || !old.designation
//           ? role
//           : old.designation,
//     }));
//   };
...
//       setMessage({
//         type: 'success',
//         text: `${docs.length} document(s) selected.`,
//       });
//     } finally {
//       setUploading(false);
//     }
...
//         (doc) => String(doc.id || doc.name) !== String(docId)
//       ),
//     }));
//   };
...
//         ? await employeeManagementApi.update(editingId, form)
//         : await employeeManagementApi.create(form);

//       setMessage({
//         type: 'success',
//         text: editingId
//           ? 'Employee updated successfully.'
//           : 'Employee created successfully. Credentials are generated below.',
//         employee,
//       });
...
//       setMessage({
//         type: 'error',
//         text: error.message || 'Unable to save employee.',
//       });
//     }
//   };
...
//     try {
//       await employeeManagementApi.toggleStatus(employee.id);
//       await loadData();
//     } catch (error) {
//       setMessage({
//         type: 'error',
//         text: error.message || 'Unable to update employee status.',
//       });
//     }
//   };
...
//     try {
//       await employeeManagementApi.remove(employee.id);
//       await loadData();
//     } catch (error) {
//       setMessage({
//         type: 'error',
//         text: error.message || 'Unable to delete employee.',
//       });
//     }
//   };

//   const previewDocument = async (employee, doc) => {
//     try {
//       await employeeManagementApi.previewDocument(employee.id, doc);
//     } catch (error) {
//       setMessage({
//         type: 'error',
...
//   const downloadDocument = async (employee, doc) => {
//     try {
//       await employeeManagementApi.downloadDocument(employee.id, doc);
//     } catch (error) {
//       setMessage({
//         type: 'error',
...
//   const title =
//     currentRole === 'Managing Director' || currentRole === 'Operations Head'
//       ? 'Employee Management'
...
//       <PageHeader
//         title={title}
//         subtitle="Hierarchy-based employee creation with mandatory document upload, generated credentials, backend persistence, and instant login access."
//       />
...
//                       {message.employee.password || 'Not changed'}
//                     </p>
//                   </div>
//                 )}
//               </div>
...
//                             {doc.size || doc.sizeBytes || '-'} · {doc.type || 'file'}
//                           </p>
...
//                     form.documents.length >= 2
//                       ? 'text-emerald-700'
//                       : 'text-amber-700'
//                   }`}
//                 >
//                   {form.documents.length}/2 minimum documents uploaded.
//                 </p>
//               </div>
...
//                   <FiUserPlus /> {editingId ? 'Update Employee' : 'Create Employee'}
//                 </PrimaryButton>
...
//                         {employee.password ? employee.password : 'Hidden'}
//                       </p>
//                     </td>
...
//                             key={doc.id || doc.name}
//                             type="button"
//                             onClick={() => previewDocument(employee, doc)}
...
//                       <div className="mt-2 flex flex-wrap gap-2">
//                         {(employee.documents || []).slice(0, 2).map((doc) => (
//                           <button
//                             key={`download-${doc.id}`}
//                             type="button"
//                             onClick={() => downloadDocument(employee, doc)}
...
const managerExecutiveFallback = (role, department) => {
  if (!isManagerRole(role)) return [];

  const dept = department || getRoleDepartment(role);

  return dept ? [`${dept} Executive`] : [];
};

const buildRoleOptions = (role, department, apiOptions = [], selectedRole = '') => {
  const frontendOptions = getCreatableRoles(role) || [];

  return unique([
    ...apiOptions,
    ...frontendOptions,
    ...managerExecutiveFallback(role, department),
    selectedRole,
  ]);
};
...
function formatDocumentSize(doc) {
  if (!doc) return '-';
  if (doc.size) return doc.size;
  if (doc.sizeBytes) return `${Math.round(Number(doc.sizeBytes) / 1024)} KB`;
  if (doc.file?.size) return `${Math.round(Number(doc.file.size) / 1024)} KB`;
  return '-';
}

function openLocalDocument(doc) {
  if (doc?.file) {
...
function downloadLocalDocument(doc) {
  if (doc?.file) {
    const url = URL.createObjectURL(doc.file);
...
export default function EmployeeManagementPanel() {
  ...
  const canManage =
    Boolean(apiCanManage) || isSuperUserRole(currentRole) || isManagerRole(currentRole);
  ...
  const roleOptions = useMemo(
    ...
    [apiRoleOptions, currentDepartment, currentRole, editingId, form.role]
  );
  ...
  const loadData = async ({ resetForm = false } = {}) => {
    try {
      ...
      const resolvedRole = data.currentRole || user?.role || '';
      const resolvedDepartment =
        data.currentDepartment || user?.department || getRoleDepartment(resolvedRole);

      const mergedRoleOptions = buildRoleOptions(
        resolvedRole,
        resolvedDepartment,
        data.roleOptions || []
      );

      setApiCanManage(Boolean(data.canManage));
      setCurrentRole(resolvedRole);
      setCurrentDepartment(resolvedDepartment);
      setApiRoleOptions(data.roleOptions || []);
      setEmployeesRaw(data.employees || []);

      if (resetForm) {
        const firstRole = mergedRoleOptions[0] || 'Sales Executive';
        setForm(blankEmployee(firstRole));
        setEditingId(null);
      }
    } catch (error) {
  ...
  useEffect(() => {
    loadData({ resetForm: true });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  useEffect(() => {
    if (!editingId && roleOptions.length && !roleOptions.includes(form.role)) {
      const role = roleOptions[0];
      setForm(blankEmployee(role));
    }
  }, [editingId, form.role, roleOptions]);

  const employees = useMemo(() => {
    const scopedEmployees = isSuperUserRole(currentRole)
      ? employeesRaw
      : employeesRaw.filter(
          (employee) =>
            employee.department === currentDepartment &&
            (!employee.role || isExecutiveRole(employee.role))
        );
  ...
  }, [currentDepartment, currentRole, employeesRaw, search]);
  ...
  const setRole = (role) => {
    setForm((old) => ({
      ...
      role,
      department: getRoleDepartment(role),
      designation: old.designation === old.role || !old.designation ? role : old.designation,
    }));
  };
  ...
  const handleDocuments = async (event) => {
    ...
    const rejected = files.filter(
      (file) =>
        !validTypes.includes(file.type) && !/\.(pdf|jpe?g|png)$/i.test(file.name)
    );
  ...
  const saveEmployee = async (event) => {
    ...
    if (!form.department) {
      setMessage({
        type: 'error',
        text: 'Department is missing. Please select role again.',
      });
      return;
    }
  ...
  const editEmployee = (employee) => {
    ...
    const role = employee.role || roleOptions[0] || 'Sales Executive';
    ...
    setForm({
      ...
      mobileNumber: employee.mobileNumber || employee.phone || '',
      address: employee.address || '',
      department: employee.department || getRoleDepartment(role),
      role,
  ...
  const previewFormDocument = async (doc) => {
    try {
      if (doc.serverDocument && editingId) {
        await employeeManagementApi.previewDocument(editingId, doc);
        return;
      }

      openLocalDocument(doc);
    } catch (error) {
      setMessage({
        type: 'error',
        text: error.message || 'Unable to preview document.',
  ...
  const downloadFormDocument = async (doc) => {
    try {
      if (doc.serverDocument && editingId) {
        await employeeManagementApi.downloadDocument(editingId, doc);
        return;
      }

      downloadLocalDocument(doc);
    } catch (error) {
      setMessage({
        type: 'error',
        text: error.message || 'Unable to download document.',
  ...
  const title = isSuperUserRole(currentRole)
    ? 'Employee Management'
    : `${currentDepartment} Executive Management`;

  const formTitle = editingId
    ? 'Edit Employee Record'
    : isSuperUserRole(currentRole)
      ? 'Create Department Manager'
      : `Create ${currentDepartment} Executive`;

  const formSubtitle = isSuperUserRole(currentRole)
    ? 'MD and Operational Head can create Sales, Marketing, CRM, Accounts, and HR Managers.'
    : 'Department managers can create only their own executives.';
          ...
          <SectionCard title={formTitle} subtitle={formSubtitle}>
            ...
            <form onSubmit={saveEmployee} className="space-y-5">
              ...
              <div className="rounded-[24px] border border-dashed border-[#0B7A8F]/40 bg-[#F0FDFA] p-4">
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-xs font-bold text-slate-500">
                            {formatDocumentSize(doc)} · {doc.type || 'file'}
                          </p>

                          <div className="mt-2 flex flex-wrap gap-2">
                            <button
                              type="button"
                              onClick={() => previewFormDocument(doc)}
                            ...
                            <button
                              type="button"
                              onClick={() => downloadFormDocument(doc)}
                              className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-700"
                            >
                              <FiDownload /> Download
                            </button>
                ...
                <p
                  className={`mt-3 text-xs font-black ${
                    form.documents.length >= 2 ? 'text-emerald-700' : 'text-amber-700'
                  }`}
                >
                  {form.documents.length}/2 minimum documents uploaded.
                </p>
              ...
              <div className="rounded-2xl bg-blue-50 p-3 text-xs font-bold leading-5 text-[#0B3D91]">
                Credentials are generated automatically after creation: username{' '}
                <b>firstname.lastname</b>, password <b>FirstName@123</b>. Duplicate
                usernames become <b>firstname.lastname1</b>,{' '}
                <b>firstname.lastname2</b>.
              </div>
        ...
        >
          ...
          <div className="table-wrap">
            <table className="min-w-[1280px] w-full text-left">
              ...
              <tbody>
                {employees.map((employee) => {
                  const imageDoc = employee.documents?.find((doc) =>
                  ...
                  return (
                    ...
                    >
                      <td className="py-4 pr-4">
                        <div className="flex items-center gap-3">
                          {imageDoc?.dataUrl || imageDoc?.url ? (
                            <img
                              src={imageDoc.dataUrl || imageDoc.url}
                              alt={employee.name}
                      ...
                      <td className="py-4 pr-4">
                        <p className="font-black text-slate-900">{employee.role}</p>
