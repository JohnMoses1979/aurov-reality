...
import StatCard from '../../components/ui/StatCard.js';
import StatusBadge from '../../components/ui/StatusBadge.js';
import { salesReportsApi } from '../../services/api.js';
...
export default function SalesManagerReports() {
  ...
  const loadReports = async () => {
    try {
      ...
      const data = await salesReportsApi.getReports();

      setReports({
        stats: data.stats || reports.stats,
        executives: data.executives || [],
        tasks: data.tasks || [],
      });
    } catch (error) {
      setMessage(error.message || 'Unable to load sales reports.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      <PageHeader
        title={TITLE}
        subtitle="Real task-based reports from backend. Team Progress = Completed Tasks ÷ Total Assigned Tasks × 100."
      />
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          label="Executives"
          value={reports.stats.executives}
          icon={FiUsers}
        ...
        <StatCard
          ...
          value={reports.stats.assignedTasks}
          icon={FiBriefcase}
          trend="Total PDF tasks"
        />

        <StatCard
          label="Completed Tasks"
          value={reports.stats.completedTasks}
          icon={FiCheckCircle}
        ...
        <StatCard
          ...
          value={reports.stats.overallProgress + '%'}
          icon={FiBarChart2}
      ...
      <SectionCard
        ...
        subtitle="Calculated from actual assigned and completed tasks."
        className="mt-6"
      >
        <div className="space-y-4">
          {reports.executives.map((employee) => (
            <div
              key={employee.id}
          ...
          {!loading && reports.executives.length === 0 && (
      ...
      >
        ...
        {!loading && reports.tasks.length === 0 && (
          <p className="rounded-2xl bg-slate-50 p-4 text-sm font-bold text-slate-500">
