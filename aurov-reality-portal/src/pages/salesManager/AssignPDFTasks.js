...
// const TITLE = 'Sales Assign PDF Tasks';

// export default function SalesManagerAssignPDFTasks() {
//   const { user } = useAuth();
...
import PrimaryButton from '../../components/ui/PrimaryButton.js';
import StatusBadge from '../../components/ui/StatusBadge.js';
import { salesPdfTaskApi } from '../../services/api.js';
...
export default function SalesManagerAssignPDFTasks() {
  ...
  const loadData = async () => {
    try {
      ...
      const [executivesData, tasksData] = await Promise.all([
        salesPdfTaskApi.getExecutives(),
        salesPdfTaskApi.getTasks(),
      ]);

      setExecutives(Array.isArray(executivesData) ? executivesData : []);
      setTasks(Array.isArray(tasksData) ? tasksData : []);
    } catch (error) {
      setMessage(error.message || 'Unable to load PDF task data.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const submitTask = async (event) => {
    ...
    try {
      ...
      const assigned = await salesPdfTaskApi.assign({
  ...
  const downloadPdf = async (task) => {
    try {
      setMessage('');
      await salesPdfTaskApi.downloadPdf(task);
    } catch (error) {
