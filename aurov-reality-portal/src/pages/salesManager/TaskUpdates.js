...
import { salesTaskUpdatesApi } from '../../services/api.js';

const DEPARTMENT = 'Sales';
const TITLE = 'Sales Task Updates';

export default function SalesManagerTaskUpdates() {
  ...
  const loadTasks = async () => {
    try {
      ...
      const data = await salesTaskUpdatesApi.getUpdates();
  ...
  const downloadFile = async (task, file) => {
    try {
      setMessage('');
      await salesTaskUpdatesApi.downloadTaskFile(task.taskId, file.type, file.name);
    } catch (error) {
  ...
  const reviewTask = async (task, decision) => {
    try {
      setMessage('');
      await salesTaskUpdatesApi.reviewTask(task.taskId, decision);
      await loadTasks();
  ...
  return (
    <div>
      <PageHeader
        title={TITLE}
        subtitle={'Review submitted update files from ' + DEPARTMENT + ' executives.'}
      />

      <SectionCard
        title="Submitted Task Updates"
        subtitle="Download submitted PDFs, review executive work, and approve, reject, or close tasks."
        action={
