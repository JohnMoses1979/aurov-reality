...
import PrimaryButton from '../../components/ui/PrimaryButton.js';
import StatusBadge from '../../components/ui/StatusBadge.js';
import { salesLeadsApi } from '../../services/api.js';
...
export default function SalesManagerLeads() {
  ...
  const loadRecords = async () => {
    try {
      ...
      setMessage(error.message || 'Unable to load sales leads.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateLeadStatus = async (item, status) => {
    try {
      ...
      await salesLeadsApi.updateLeadStatus(item.recordId, status);
      await loadRecords();
  ...
  const updateBookingStatus = async (item, status) => {
    try {
      ...
      await salesLeadsApi.updateBookingStatus(item.recordId, status);
      await loadRecords();
  ...
  return (
    <div>
      <PageHeader
        title={TITLE}
        subtitle="Department customer enquiries, demo requests, bookings, and reservations from backend database."
      />
